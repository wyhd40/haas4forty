...
private function init_options($form_num) {
  ...
  if ( is_array($fsc_gb_defaults) && is_array($fsc_gb) )
    $fsc_gb = array_merge($fsc_gb_defaults, $fsc_gb);

  // multi-form  // (addded admin check)
  $fsc_max_forms = $fsc_gb['max_forms'];
  global $passed_login;
  if ($passed_login && !isset($_GET['site_settings']) && isset($_POST['submit']) && !isset($_POST['ctf_action'])) {
      if ( isset($_POST['fsc_max_forms']) && is_numeric($_POST['fsc_max_forms']) && $_POST['fsc_max_forms'] < 100  ) {
            $fsc_max_forms = $_POST['fsc_max_forms'];
      }
 }
  ...
  // array merge incase this version has added new options
  if ( is_array($fsc_opt_defaults) && is_array($fsc_opt) )
