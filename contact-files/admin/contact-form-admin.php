...
if ( $fsc_site = $this->get_option("fsc_site") ) {
 ...
 if ($passed_login && isset($_GET['site_settings']) && isset($_POST['ctf_action']) && $_POST['ctf_action'] == 'update' && $_POST['admin_pwd'] != '') {
        // flush later after set cookie
 } else {
   ...
   if ( isset($_POST['ctf_action']) && $_POST['ctf_action'] == 'update' ) {
        ...
        if ($_POST['admin_pwd'] != '') {
            $admin_pwd_upd = 'hashed_'. md5(trim($_POST['admin_pwd']));
             ...
             fsc_setcookie("fsc_verify", md5($fsc_site['admin_usr'].'%'.$admin_pwd_c), 0, $cookie_path);
             ob_end_flush(); // Flush the output from the buffer 
        }
...
if ( $fsc_opt['email_from'] != '' ) {
   ...
   if ($from_fail)  {
       echo '<div class="error">';
       echo _('ERROR: Misconfigured "Return-path address" address.');
       echo "</div>\n";
   } else {
       ...
       echo sprintf(_('Warning: "Return-path address" is not set to an address from the same domain name as your web site (%s). This can sometimes cause mail not to send, or send but be delivered to a Spam folder. Be sure to test that your form is sending email and that you are receiving it, if not, fix this setting.'), $blogdomain);
       echo "</div>\n";
       }
   }
}
?>
        ...
        <label for="fsc_email_from"><?php echo _('Return-path address (recommended):'); ?></label><br />
        <?php echo _('Set this to a real email address on the SAME domain as your web site.'); ?><br />
        <?php echo _('For best results the "Email To" and the "Return-path address" should be separate email addresses on the SAME DOMAIN as your web site.'); ?>
        ...
        <?php echo _('Return-path address sets an email header for the address the messages are sent from. Some web hosts do not allow PHP to send email unless the Return-path email address header is set. It must be set to a real email address on your site domain, or mail will NOT SEND! (They do this to help prevent spam.)'); ?>
        <?php echo _('This setting is required by Yahoo, AOL, Comcast, Dreamhost, GoDaddy, and most others now.'); ?>
        <br />
...
<br />
       ...
       if( $fsc_opt['email_from_enforced'] == 'true' && $fsc_opt['email_from'] == '') {
         echo '<div class="updated">';
         echo _('Warning: Enabling this setting requires the "Return-path address" setting above to also be set.');
         echo "</div>\n";
       }
       ?>
        ...
        <label for="fsc_email_from_enforced"><?php echo _('Enable when web host requires "Mail From" strictly tied to domain email account. (recommended, <a href="http://www.fastsecurecontactform.com/yahoo-com-dmarc-policy" target="_new">see FAQ</a>)'); ?></label>
        ...
        <?php echo _('This setting is for DMARC Compliance and is required by Yahoo, AOL, Comcast, Dreamhost, GoDaddy, many others.'); ?>
        <?php echo _('The email will appear to be from your site email address, but because the email header "Reply-to" is set as the form user\'s email address, you should be able to just hit reply and email back to the real sender. Also you will see the sender address in the message content, so it is still possible to send mail to them if the "Reply-to" is ignored by your email program.') ?><br />
        </div>
        <br />

        <label for="fsc_email_reply_to"><?php echo _('Custom Reply To (optional, rarely needed)'); ?>:</label>
        ...
        <?php echo _('Leave this setting blank for most forms because the "reply to" is set automatically to the user email. Only use this setting if you are using the form for a mailing list and you do NOT want the reply going to the form user.'); ?>
...
?>
        ...
        <?php echo _('If your form does not send any email, first try setting the "Return-path address" setting above because some web hosts do not allow PHP to send email unless the "From:" email address is on the same web domain.'); ?>
